...
- [ ] **個体単位テーブルへの正規化**（任意）… `cat_id`重複15個体をどう畳むか
    - 重複15個体のうち**少なくとも1頭は種雄牛**（`桜山` = `1267672214`。血液と骨格筋で2回genotype）。
      畳めば血統解決にも効く（`resolve_p1_cat_id` の名号法は「同名が複数行」を曖昧とみなすため、
      かつてこの重複が原因で桜山の子112頭の `p1_cat_id` が引けていなかった。現在は行単位ではなく
      (名号, cat_id) 単位で曖昧判定するよう修正済み）。
